...
$(function() {
    ...
    $(document).on('click', '.read-more-trigger', function() {
        var $this  = $(this);
        $this.hide();
        $readmore = $this.closest('.read-more-boundary').find('.read-more').fadeIn();
    });
    ...
    window.Parsley.addValidator('websiteCheck', {
        validateString: function(value) {
			//Strip .com for comparison
			value = value.replace(/.com/g,'');
			
			//One URL only
            //return ( value.indexOf( window.location.hostname.replace('www.','') ) === -1) ? value : false;
			
			//Validate all domains
			if ((value.indexOf("firstpage") !== -1) || (value.indexOf("appscore.au") !== -1) || (value.indexOf("removify.au") !== -1) ){
				return false;
			}
			else{
				return true;
    ...
    $('.quote-form form').on('keyup keypress', function(e) {
                ...
                $.ajax({
                    ...
                    success : function( data ) {
                        ...
                        // If Thank You step exists
                            ...
                            if ((typeof gaIdLabel !== 'undefined') && (typeof gtag !== 'undefined')) {
    ...
    $(".btn-comment-reply").click(function() {
        var $this  = $(this);
        $this.closest('.blog-comment').children('.comment-box').fadeToggle('fast', 'linear');
    });
    ...
    if ($(".mform").length > 0) {
            ...
            current.animate({opacity: 0}, {
                step: function(now, mx) {
                    ...
                    // Previous
                        ...
                        destination.css({'transform': 'scale('+scale+')', 'opacity': opacity});
                    }
                }, 
                duration: 800, 
                complete: function(){
        ...
        $(".mform-next").click(function() {
            ...
            $(".mform-progressbar li").eq($(".mform fieldset").index(destination)).addClass("active");
        ...
        $(".mform-prev").click(function() {
            ...
            $(".mform-progressbar li").eq($(".mform fieldset").index(current)).removeClass("active");
        ...
        $(document).on('click', '.mform-progressbar li.active', function() {
            ...
            destination = $('.mform fieldset').eq($(".mform-progressbar li").index($this));
        ...
        $(".mform-selection .mform-select").click(function(){
            ...
            if ($parent.hasClass('single-select')) {
                ...
                $parent.next('.mform-btn-nav').prop("disabled", $selectedItems.length == 0);
            }
            return false;
        });

        // Rangeslider
        $('.mform input[type="range"]').rangeslider({
            polyfill: false,
            onInit: function() {
            ...
            $handle[0].textContent = '$' + this.value + (this.value == 250000 ? '+' : '');
        });

        // Form Submit
        if($('#session-booking-form').length > 0){
            $('#session-booking-form').parsley().on('form:submit', function() {           
                // Capture User Details
                var booking_date = $('#date_booking1').val();
                var booking_time = $('select[name=rangetime1] option:selected').val();

                var n_booking_time = booking_time.substr(0, booking_time.indexOf(" - "));
                var n_booking_date = moment(booking_date + ' ' + n_booking_time, 'DD.MM.YYYY HH:mm').format("MMMM DD YYYY HH:mm:ss");
                var n_booking_date_text = moment(booking_date, 'DD.MM.YYYY').format("DD MMMM YYYY");
                ...
                for (var i = 0; i < selections.length; i++) {
                    ...
                    var selectedValues = $selection.find('.selected .mform-select-value').map(function() { return $.trim($(this).text()); }).get();
                    data[ $selection.attr('data-field') ] = selectedValues.join(", ");
                }

                // Submit to WPGlucose Form via Ajax
                if (!!booking_date) { // It will get rejected anyway by the next step
                    $.ajax({
                        type: 'POST',
                        url: wpSiteUrl + "/action/hubspot/submit.php",
                        data: data
                ...
                $('.wpbc_structure_form input[name=marketing_used1]').val( data.wpg_marketing_used );
                $('.wpbc_structure_form input[name=describe_me1]').val( data.wpg_describe_me );
    ...
    if($('#new-session-form').length > 0){
        $('#new-session-form').parsley().on('form:submit', function() {
            var $this = $(this)[0].$element;

            // Add Spinner
            $this.find('.mform-btn-submit').append(' <i class="fa fa-spinner fa-spin"></i>');
    ...
    let allVimeoReadMoreCollapses = $('.vimeo-read-more-collapse');
    let allVimeoSigns = $('.vimeo-sign');

    $('.vimeo-toggle').on('click', function() {
                
        var wasPreviouslyOpened = false;
        var currentReadMoreCollapse = $('#' + $(this).attr('data-target'));
        var currentVimeoSign = $(this).find('.vimeo-sign');

        if(currentReadMoreCollapse.css('display') === 'block') {
            wasPreviouslyOpened = true;
        }

        allVimeoReadMoreCollapses.slideUp();
        allVimeoSigns.html('+');
    
        if(!wasPreviouslyOpened)
        {
            currentReadMoreCollapse.slideDown();
            currentVimeoSign.html('-');
        }
    });

    $('.vimeo-read-more-collapse').on('click', function() {
        $(this).slideUp();
    });

    // Video Production Page - Maginic Popup

    $('.page-video-production-photography .photo').magnificPopup({
        type: 'image',
        gallery:{
            enabled:true
        }
